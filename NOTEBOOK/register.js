...
//      console.log(el);
  ...
  function createPage(userLogName, userLogPass) {
    ...
    console.log(iButtonSubmit);
    iButtonSubmit.onclick = doFormSubmit2(event);
    console.log(iButtonSubmit);
//    iButtonSubmit.addEventListener("Click", doFormSubmit2(event));
//    iButtonSubmit.addEventListener("Click", doFormSubmit2(event));
/*
    function doFormSubmit(event) {
//      let formObj = event.target.closest("form");
      let submitObj = signForm.elements.submit;
      for(var i = 0; i < signForm.elements.length; i++){
        el = signForm.elements[i];
        console.log(el);
        if (!el.value || !el.validity.valid) return false; // submit need to display setValidity, return needed
      }
      event.preventDefault();
  ...
  obj.doFormSubmit2 = doFormSubmit2;
